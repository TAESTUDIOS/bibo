...
export async function POST(req: Request) {
  try {
    ...
    // Fallback to plain text rendering if JSON wasn't provided
    const finalText = apiText ?? mockAdvice(userText);
    const msg = { id: uid("m"), role: "assistant" as const, text: finalText, timestamp: Date.now() };
    return NextResponse.json({ ok: true, message: msg });
  } catch (e: any) {
