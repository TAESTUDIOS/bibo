...
export async function GET() {
  try {
    await ensureTable();
    const sql = getDb();
    const rows = await sql`SELECT id, role, text, ritual_id, buttons, metadata, emotion_id, emotion_tone, timestamp_ms FROM messages ORDER BY timestamp_ms DESC LIMIT 100`;
    const messages = rows
      .map((r: any) => ({ id: r.id, role: r.role, text: r.text, ritualId: r.ritual_id || undefined, buttons: r.buttons || [], metadata: r.metadata || {}, emotionId: r.emotion_id || undefined, emotionTone: r.emotion_tone || undefined, timestamp: Number(r.timestamp_ms) }))
      .reverse();
...
export async function POST(req: Request) {
  try {
    ...
    if (incomingRole && incomingText) {
      ...
      await sql`INSERT INTO messages (id, role, text, ritual_id, buttons, metadata, emotion_id, emotion_tone, timestamp_ms)
                VALUES (${id}, ${incomingRole}, ${incomingText}, ${incomingRitualId || null}, ${buttonsJson}::jsonb, ${metadataJson}::jsonb, ${incomingEmotionId || null}, ${incomingEmotionTone || null}, ${incomingTs})
