...
export default function EmotionHeader({ emotion }: Props) {
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-lg border px-3 py-2 ${decor.headerBg}`}
      aria-label={`${emotion.title} emotion header`}
    >
      <div className={`relative h-10 w-10 shrink-0 rounded-full bg-black/10 ring-2 ${decor.iconRing}`}>
        <Image
          src={emotion.image}
